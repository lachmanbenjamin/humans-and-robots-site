/**
 * H&R Lead Pipeline — Google Apps Script Web App
 * Bound to the "H&R Master Leads" spreadsheet.
 ...
 *
 * SETUP (one time):
 *  1. Open the Master Sheet → Extensions → Apps Script. Paste this file as Code.gs.
 ...
 *     Click Deploy, authorize, copy the Web app URL (ends in /exec).
 *  4. Put that /exec URL into both website forms (see LEAD_ROUTING.md).
 *
 * To verify: run testLead() from the editor, or POST a JSON body to the /exec URL.
 */
...
function normalize_(d) {
  d = d || {};
  // Decide source: explicit field, else infer from presence of trade/zip
...
function smsBen_(l) {
  ...
  var from = props.getProperty('TWILIO_FROM') || '+12245071823';
